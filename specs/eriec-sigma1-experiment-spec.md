...
### VP-META-005 — Σ1 diversity 飽和診断（2026-07-20 自律起票）

初回実走で selected/null の `diversity_median` が全 replicate で 1.0 となった原因を、
評価規則を事後変更せずに診断する。既存 `sigma_selection.jl`、seed、tol、世代数、
`Inf → M_f` 二段 adapter は変更しない。

- Lean anchor `ERIEC.MetaSelection.DiversityAuditPure`: 診断前後の個体列について、
  protected observation trace が同一である read-only 境界。
- Julia API `check_sigma1_diversity_resolution`: 全個体を固定 size ごとに層別化し、
  各 size 内の全 unordered pair の projection distanceについて、pair数、中央値、
  Tukey hinges による IQR、厳密非零率を返す。
- 同時に、既存 size=2 の i_ref に対する raw distance が非有限となり `M_f` に置換された
  個体数・比率を全体およびsize別に記録する。
- selected/null 最終archiveと初期64個体を別々に報告する。数値pass/fail閾値は追加せず、
  診断結果から新しいdiversity定義を採用する場合は別VP・別実験として事前登録する。
- 診断実走は既存 seed `Xoshiro(20260720+k)`、k=0..9を再利用し、新規seedを追加しない。

contract: `meta.sigma1_diversity_audit`。depends_on: VP-META-004。
...
4. VP-META-005 は初回実走後のread-only診断。既存実験の判定・certificateを変更しない。
